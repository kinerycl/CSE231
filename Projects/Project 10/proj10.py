...
#       print board
#       gets piece based on turn
#       prompts for a command
#           if 'exit', breaks loop
#           if 'hint', gives hint
#           if 'pass', passes turn
#           else, places piece on board and flips streaks
#    displays winner message or draw message if no winner
###########################################################
import reversi
import string
...
"""
Program is the game Reversi. The game is restricted to two players: one with 
black pieces and the other with white pieces. Players take their turn by placing
a piece on the board. The piece turns opponent's pieces to current player's 
color in a straight line which are bounded by the piece just placed and another
of the current player's color. Game ends when there are no more moves or the 
board is full. The winner is decided based on most pieces on board.
"""
...
def indexify(position):
    ...
    for cha, val in enumerate(LETTERS, 0):
        row_dict[val] = cha
    return (row_dict[position[0]], int(position[1:])-1) 
...
def get_all_streaks(board, row, col, piece_arg):
    """
    Iterates through all potential streaks on board based on direction and 
    returns a dictionary of all the streaks. Keys are direction of streaks and 
    values are the postions of the streaks.
    """
...
def place_and_flip(board, row, col, piece):
    """
    Places a piece based on the given column and row. Flips pieces based on the
    streak postions of placed piece's position. If an ocuppied position input, 
    raises ValueError. If an invaild position input, raises ValueError. If 
    position does not have any streaks for piece, raises ValueError.
    """
    color = piece.color()[0].upper()
    pos = deindexify(row, col)
    cap_cells = get_all_capturing_cells(board, piece)
    
    # Error messages
    pre_error_msg = "Can't place {} at '{}',".format(color, pos)
    error_msg1 = pre_error_msg \
    + " invalid position. Type 'hint' to get suggestions."
    error_msg2 = pre_error_msg \
    +" already occupied. Type 'hint' to get suggestions."
    error_msg3 = pre_error_msg \
...
def choose_color():
    ...
    print("You are '{:s}' and your opponent is '{:s}'.".format(my_color, \
          opponent_color))
