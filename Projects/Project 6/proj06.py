...
def main():
    ...
    while True:
        ...
        chrom_name = chrom_name[:3].lower()+chrom_name[3:].lower() 
        if chrom_name in CHROMOSOMES:
            print("Chromosome Length")
            print("{:<11s}{:>9s}{:>9s}".format("chromosome", "mean", "std-dev"))
            display_data(compute_gene_length\
                         (extract_chromosome(gene_list, chrom_name)),\
                         chrom_name)
            ...
            for i in CHROMOSOMES:
                i = str(i)
                display_data(compute_gene_length(extract_chromosome\
                                                 (gene_list, i)), i)
